...
class ReadyadscompanyController extends GetxController {
  ReadyadscompanyController();

  final state = ReadyadscompanyState();
   void changePageState() =>
      state.gettingState.value = !state.gettingState.value;

  void getPage() async {
    changePageState();
    var result = await state.createAdHttp.getWaitingAds(api: getReadyAdsCompany);
