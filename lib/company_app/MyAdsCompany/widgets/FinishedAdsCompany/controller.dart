...
class FinishedadscompanyController extends GetxController {
  FinishedadscompanyController();

  final state = FinishedadscompanyState();
    void changePageState() =>
      state.gettingState.value = !state.gettingState.value;

  void getPage() async {
    changePageState();
    var result = await state.createAdHttp.getWaitingAds(api: getDoneAdsCompany);
